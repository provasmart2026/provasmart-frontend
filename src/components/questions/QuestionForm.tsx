...
export function QuestionForm({
    ...
    return (
        <form className="question-form" onSubmit={submit} aria-busy={saving || options.creating || Boolean(options.loading)}
              aria-describedby={error ? `${id}-error` : undefined}>
            <fieldset disabled={saving}>
                <legend>Dados da questão</legend>
                <QuestionSubjectFields id={id} subjectId={value.subjectId} options={options}
                    onSubjectChange={(subjectId) => onChange({...value, subjectId})}/>
                <label htmlFor={`${id}-statement`}>Enunciado</label>
                <textarea id={`${id}-statement`} required rows={5} value={value.statement}
            ...
            <fieldset disabled={saving} aria-label="Explicação da questão">
                ...
                <textarea id={`${id}-explanation`} required rows={4} value={value.explanation}
