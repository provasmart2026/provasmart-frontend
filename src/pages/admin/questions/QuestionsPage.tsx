...
export function QuestionsPage() {
    ...
    async function toggleActive(question: Question) {
        ...
        setStatusError(null)
        try {
            if (question.active) await questionService.deactivate(question.id)
            else await questionService.activate(question.id)
        ...
        <section className="admin-questions" aria-labelledby="questions-title">
            <div className="questions-heading">
                ...
                <button
                    ...
                    onClick={() => navigate('/admin/questions/new', { state: { page } })}
                >
                    Nova questão
                </button>
            </div>
                    ...
                    data && (
                        <>
                                ...
                                <QuestionTable
                                    ...
                                    onEdit={
                                        changingStatus
                                            ? undefined
                                            : (question) =>
                                                  navigate(`/admin/questions/${encodeURIComponent(question.id)}/edit`, {
                                                      state: { page },
                                                  })
                                    }
                            ...
                            <nav className="questions-pagination" aria-label="Paginação de questões">
                                <button
                                    type="button"
                                    disabled={changingStatus || data.first || data.number === 0}
                                ...
                                <span className="questions-pagination-status" role="status">
                                    Página {data.totalPages === 0 ? 0 : data.number + 1} de {data.totalPages}
                                </span>
                                <button
                                    type="button"
