...
const input: QuestionInput = {
    ...
    alternatives: ['A', 'B', 'C', 'D', 'E'].map((letter, index) => ({
        letter, text: `Resposta ${index + 1}`, correct: index === 3,
    })),
}
const question: Question = {
    ...input, id: 'question-1', subjectName: 'Matemática', active: true,
    alternatives: input.alternatives.map((item) => ({...item, id: item.letter})),
    createdAt: '2026-09-13T12:00:00', updatedAt: null,
}
...
describe('Administração de questões', () => {
    beforeEach(() => {
        ...
        catalogRequest.mockReset().mockImplementation(async ({url}) => {
            ...
            if (url === '/disciplines/exam-area/MATEMATICA') return {
                data: [
                    {id: 'math', name: 'Álgebra', examArea: 'MATEMATICA'},
                    {id: 'geometry', name: 'Geometria', examArea: 'MATEMATICA'},
                ]
            }
            if (url === '/disciplines/exam-area/LINGUAGENS') return {
                data: [{
                    id: 'portuguese',
                    name: 'Português',
                    examArea: 'LINGUAGENS'
    ...
    it('carrega a edição com GET, salva com PUT e preserva a página da listagem', async () => {
        ...
        input.alternatives.forEach((item) => expect(screen.getByLabelText(`Alternativa ${item.letter}`)).toHaveValue(item.text))
        ...
        expect(request).toHaveBeenNthCalledWith(4, {
            url: '/questions/question-1', method: 'PUT', data: {...input, statement: 'Enunciado revisado'},
        })
    ...
    ] as const)('altera o status active=%s via PATCH e recarrega a tabela', async (active, action, endpoint, status) => {
        const request = mockQuestionRequests()
    ...
    it('bloqueia envios repetidos enquanto salva e permite cancelar sem salvar', async () => {
        ...
        const request = mockQuestionRequests().mockImplementationOnce(() => new Promise((resolve) => {
            finish = resolve
        }))
        open('/admin/questions/new')
        await fillForm()
    ...
    it('cria e seleciona um assunto e envia somente seu ID no cadastro da questão', async () => {
        const request = mockQuestionRequests().mockResolvedValueOnce({data: question}).mockResolvedValueOnce({data: page([question])})
        open('/admin/questions/new')
        await fillForm()
    ...
    it.each(['areas', 'disciplines', 'subjects'])('mostra erro e permite repetir a consulta de %s', async (step) => {
        ...
        await screen.findByRole('option', {name: step === 'areas' ? 'Matemática' : step === 'disciplines' ? 'Álgebra' : 'Soma'})
    ...
    it('ignora respostas atrasadas de uma área que deixou de estar selecionada', async () => {
        ...
        catalogRequest.mockImplementationOnce(() => new Promise((resolve) => {
            finish = resolve
        }))
        fireEvent.change(screen.getByLabelText('Área'), {target: {value: 'MATEMATICA'}})
    ...
    it('não redireciona após salvar quando o usuário já saiu do formulário', async () => {
        let finish!: (value: {data: Question}) => void
        mockQuestionRequests().mockImplementationOnce(() => new Promise((resolve) => { finish = resolve }))
        open('/admin/questions/new')
        await fillForm()
        fireEvent.click(screen.getByRole('button', {name: 'Salvar questão'}))
        fireEvent.click(screen.getByRole('link', {name: 'Início'}))
        await screen.findByRole('heading', {name: /cada questão conta/i})
        await act(async () => { finish({data: question}) })
    ...
    it.each(['/disciplines/exam-area/LINGUAGENS', '/subjects/discipline/geometry'])(
        'preenche a edição mesmo quando falha um catálogo não relacionado: %s', async (failedUrl) => {
            const defaultCatalog = catalogRequest.getMockImplementation()!
            catalogRequest.mockImplementation((config) => config.url === failedUrl
                ? Promise.reject(new Error('Falha em outro catálogo')) : defaultCatalog(config))
    ...
    it('bloqueia o salvamento e permite repetir quando o catálogo do assunto da edição falha', async () => {
        const defaultCatalog = catalogRequest.getMockImplementation()!
        catalogRequest.mockImplementation((config) => config.url === '/subjects/discipline/math'
            ? Promise.reject(new Error('Falha')) : defaultCatalog(config))
