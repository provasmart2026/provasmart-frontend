...
export function UsersPage() {
    ...
    async function handleAction(user: UserResponse, action: UserAction) {
        ...
        if (
            action === 'delete' &&
            !window.confirm('Esta exclusão é definitiva e não poderá ser desfeita. Deseja continuar?')
        )
            return
        ...
        setActionError(null)
        try {
            await userService[action](user.id)
                                            ...
                                            {user.role === 'ESTUDANTE' ? (
                                                <div className="users-actions">
                                                    <button
                                                        ...
                                                        onClick={() =>
                                                            handleAction(user, user.active ? 'deactivate' : 'activate')
                                                        }
                                                    >
                                                        {busyUserId === user.id
                                                            ? 'Processando...'
