...
async function saveAndConfirmAnswer(simulationId: string, questionId: string, alternativeId: string) {
    ...
    const response = await simulationsApi.answer(simulationId, questionId, alternativeId)
    const confirmedSimulation = hasPersistedAnswer(response) ? response : await simulationsApi.get(simulationId)
    ...
    async function saveLatestSelectedAlternative(alternativeId: string) {
        ...
        try {
            while (mounted.current && latestSelectedAlternativeId.current !== null) {
                ...
                if (latestSelectedAlternativeId.current === alternativeIdToSave) latestSelectedAlternativeId.current = null
            }
        } catch {
                        ...
                        <div className="simulation-navigation">
                            ...
                            {currentIndex < questions.length - 1 ?
                                <button className="primary-button" type="button" disabled={busy}
                                        onClick={() => goToQuestion(currentIndex + 1)}>Próxima</button> :
