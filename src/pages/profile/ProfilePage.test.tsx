...
const user: UserResponse = {
    id: 'private-user-uuid', name: 'Aline Souza', email: 'aline@exemplo.com',
    role: 'ESTUDANTE', active: true, deletionRequested: false, deletionRequestedAt: null,
    termsVersion: 'termos-1', termsAcceptedAt: '2026-09-20T10:30:00',
    privacyVersion: 'privacidade-2', privacyAcceptedAt: '2026-09-21T11:45:00',
...
describe('Meu perfil', () => {
    ...
    it('não envia PATCH quando a confirmação é cancelada', async () => {
        ...
        expect(confirm).toHaveBeenCalledWith('Deseja realmente solicitar a exclusão da sua conta? Um administrador precisará concluir a exclusão definitiva.')
    ...
    it.each(['ADMIN', 'ESTUDANTE'] as const)('solicita exclusão para %s sem body e atualiza o perfil', async (role) => {
        ...
        request.mockReturnValueOnce(pending.promise).mockResolvedValueOnce({data: {
            ...user, role, deletionRequested: true, deletionRequestedAt: '2026-09-25T14:30:00',
        }})
        ...
        expect(screen.getByText('Sua solicitação de exclusão foi registrada e aguarda processamento.')).toBeInTheDocument()
    ...
    it.each([false, true])('aguarda o GET e respeita deletionRequested=%s retornado pelo backend', async (deletionRequested) => {
        const request = await open()
    ...
    it('mantém o botão disponível após falha do PATCH', async () => {
        ...
        vi.spyOn(window, 'confirm').mockReturnValue(true)
        fireEvent.click(screen.getByRole('button'))
        expect(await screen.findByRole('alert')).toHaveTextContent('Não foi possível solicitar a exclusão da conta. Tente novamente.')
        expect(screen.getByRole('button', {name: 'Solicitar exclusão da conta'})).toBeEnabled()
        expect(screen.queryByText('Exclusão solicitada')).not.toBeInTheDocument()
        expect(request).toHaveBeenCalledTimes(2)
    })
    ...
    it('preserva os últimos dados do backend e informa falha ao atualizar após o PATCH', async () => {
        ...
        fireEvent.click(screen.getByRole('button'))
        await waitFor(() => expect(request).toHaveBeenCalledTimes(3))
        expect(await screen.findByRole('alert')).toHaveTextContent('Sua solicitação foi registrada, mas não foi possível atualizar seus dados agora.')
        expect(screen.queryByText('Exclusão solicitada')).not.toBeInTheDocument()
        expect(screen.getByRole('button', {name: 'Solicitar exclusão da conta'})).toBeEnabled()
    })
})
