...
function LocationInfo() {
    return (
        <div data-testid="location">
            {useLocation().pathname} {useNavigationType()}
        </div>
    )
}
...
describe('tratamento global de autenticação HTTP', () => {
    ...
    describe.each([401, 403])('sem token, HTTP %s', (status) => {
        it.each(['/auth/login', '/auth/verify-2fa', '/auth/forgot-password', '/auth/reset-password', '/users'])(
            '%s apenas lança ApiError',
            async (path) => {
                renderApp('/login')
                const dispatch = vi.spyOn(window, 'dispatchEvent')
                const adapter = errorAdapter(status)
                await act(async () => {
                    await expect(apiRequest(path, { method: 'POST', adapter })).rejects.toMatchObject({
                        name: 'ApiError',
                        status,
                    })
                })
                ...
                expect(
                    dispatch.mock.calls.filter(([event]) => [authChangedEvent, authRedirectEvent].includes(event.type))
                ).toHaveLength(0)
    ...
    it('saveSession publica apenas a sessão final e logout manual continua limpando', () => {
        ...
        const listener = () =>
            observed.push(Boolean(localStorage.getItem(tokenKey) || sessionStorage.getItem(tokenKey)))
