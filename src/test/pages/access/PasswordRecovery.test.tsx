...
describe('recuperação de senha', () => {
    ...
    it.each([null, {}, { email: '' }, { email: 12 }, { email: 'invalido' }, { email: 'a b@exemplo.com' }])(
        'redireciona state inválido %j',
        (state) => {
    ...
    it.each([400, 401, 403, 422, 500, 0])('trata erro %s na redefinição sem detalhes internos', async (status) => {
        ...
        expect(screen.getByRole('link', { name: 'Solicitar novo código' })).toHaveAttribute(
            'href',
            '/esqueci-minha-senha'
        )
    })
