...
const student: UserResponse = {
    ...
    termsVersion: '1',
    termsAcceptedAt: '2026-09-19T09:15:00',
    privacyVersion: '1',
    privacyAcceptedAt: '2026-09-19T09:15:00',
}
...
describe('Administração de usuários', () => {
    ...
    it('busca a primeira página e exibe dados e ações conforme perfil e estado', async () => {
        ...
        for (const user of [student, inactive, requested, admin])
        ...
        if (action === 'Ativar') expect(confirm).not.toHaveBeenCalled()
            ...
            expect(confirm).toHaveBeenCalledWith(
                action === 'Desativar'
                    ...
                    : 'Esta exclusão é definitiva e não poderá ser desfeita. Deseja continuar?'
            )
    })
    ...
    it('não atualiza a página após desmontar durante uma ação', async () => {
        ...
        const request = vi
            .spyOn(api, 'request')
            .mockResolvedValueOnce({ data: page([inactive]) })
            .mockReturnValueOnce(
                new Promise((done) => {
                    resolve = done
                })
